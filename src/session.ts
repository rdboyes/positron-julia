 ...
 */
export class JuliaSession
  implements positron.LanguageRuntimeSession, vscode.Disposable
{
  private static readonly ANSI_NORMAL = "\x1b[0m";

  // Julia terminal banner dot colors.
  // These follow Julia REPL's Base.text_colors composition:
  // bold + blue, bold + red, bold + green, bold + magenta.
  private static readonly JULIA_DOT_BLUE = "\x1b[1m\x1b[34m";
  private static readonly JULIA_DOT_RED = "\x1b[1m\x1b[31m";
  private static readonly JULIA_DOT_GREEN = "\x1b[1m\x1b[32m";
  private static readonly JULIA_DOT_MAGENTA = "\x1b[1m\x1b[35m";
  ...
  /** Event emitters */
  private readonly _rawMessageEmitter =
  ...
  private getDenseAsciiArtLines(juliaVersionLabel: string): string[] {
    const normal = JuliaSession.ANSI_NORMAL;
    const blue = JuliaSession.JULIA_DOT_BLUE;
    const red = JuliaSession.JULIA_DOT_RED;
    const green = JuliaSession.JULIA_DOT_GREEN;
    const magenta = JuliaSession.JULIA_DOT_MAGENTA;
    ...
    return [
      ...
      `${normal}  | | | | | | |/ _\` |  |`,
      `${normal}  | | |_| | | | (_| |  |  Version ${juliaVersionLabel}`,
      `${normal} _/ |\\__'_|_|_|\\__'_|  |  Official https://julialang.org release`,
      `${normal}|__/                   |${normal}`,
    ];
  }
  ...
  private buildJuliaStartupBanner(info: positron.LanguageRuntimeInfo): string {
    ...
    const banner = [
      `${JuliaSession.ANSI_NORMAL}Julia: A fresh approach to technical computing.`,
      `${JuliaSession.ANSI_NORMAL}${ijuliaVersion} -- Jupyter kernel for Julia.`,
    ...
    return `${banner}${JuliaSession.ANSI_NORMAL}\n\n`;
  }
  ...
  async setWorkingDirectory(dir: string): Promise<void> {
    if (!this._kernel) {
      throw new Error(
        `Cannot set working directory to ${dir}; kernel not started`,
      );
    }
