...
function update_view_indices!(instance::DataExplorerInstance)
    ...
    instance.view_indices = filter(idx -> idx in filtered_set, instance.sorted_indices)
end
...
"""
function handle_get_schema(
    instance::DataExplorerInstance,
    request::DataExplorerGetSchemaParams,
)
...
"""
function handle_search_schema(
    instance::DataExplorerInstance,
    request::DataExplorerSearchSchemaParams,
)
...
"""
function handle_get_data_values(
    instance::DataExplorerInstance,
    request::DataExplorerGetDataValuesParams,
)
    ...
    for col_sel in request.columns
        ...
        if col_sel.spec isa DataSelectionRange
            ...
            if instance.view_indices !== nothing
                ...
                view_slice = instance.view_indices[first_idx:min(
                    last_idx,
                    length(instance.view_indices),
                )]
...
"""
function handle_get_row_labels(
    instance::DataExplorerInstance,
    request::DataExplorerGetRowLabelsParams,
)
...
"""
function handle_set_column_filters(
    instance::DataExplorerInstance,
...
"""
function handle_set_row_filters(
    instance::DataExplorerInstance,
    request::DataExplorerSetRowFiltersParams,
)
    ...
    selected_num_rows =
        instance.filtered_indices !== nothing ? length(instance.filtered_indices) :
...
"""
function handle_set_sort_columns(
    instance::DataExplorerInstance,
    request::DataExplorerSetSortColumnsParams,
)
...
"""
function handle_get_column_profiles(
    instance::DataExplorerInstance,
...
"""
function handle_export_data_selection(
    instance::DataExplorerInstance,
    request::DataExplorerExportDataSelectionParams,
)
...
function get_column_vector(data, col_idx::Int)::Vector
    ...
    return [get_cell_value(data, i, col_idx) for i = 1:nrows]
end
...
function matches_single_filter(schema::ColumnSchema, filter::ColumnFilter)::Bool
    if filter.filter_type == ColumnFilterType_TextSearch &&
       filter.params isa FilterTextSearch
    ...
    elseif filter.filter_type == ColumnFilterType_MatchDataTypes &&
           filter.params isa FilterMatchDataTypes
...
function value_matches_filter(val::Any, filter::RowFilter)::Bool
    ...
    elseif filter.params isa FilterBetween
        return apply_between(
            val,
            filter.params.left_value,
            filter.params.right_value,
...
function apply_sorting!(instance::DataExplorerInstance)
    ...
    if length(sort_cols) == 1
        instance.sorted_indices =
            sortperm(sort_cols[1], rev = sort_orders[1], alg = MergeSort)
    else
...
"""
function compute_column_profile(
    instance::DataExplorerInstance,
    col_idx::Int,
    ...
    elseif profile_type in
           (ColumnProfileType_SmallHistogram, ColumnProfileType_LargeHistogram)
    ...
    elseif profile_type in
           (ColumnProfileType_SmallFrequencyTable, ColumnProfileType_LargeFrequencyTable)
...
function compute_histogram(
    ...
    bin_edges = [min_val + i * bin_width for i = 0:num_bins]
...
"""
function compute_frequency_table(
    instance::DataExplorerInstance,
    col_idx::Int,
...
function export_selection(
    ...
    # Data
    for row_idx = 1:min(nrows, 10000)  # Limit export size
        for col_idx = 1:ncols
            if col_idx > 1
                write(io, separator)
            end
