...
@testset "Data Explorer - Sorting" begin
    ...
    @testset "apply_sorting! - Multi-Column" begin
        df = DataFrame(category = ["A", "B", "A", "B", "A"], value = [3, 1, 2, 4, 1])
        instance = Positron.DataExplorerInstance(df, "test")
...
@testset "Data Explorer - Sorting Correctness" begin
    @testset "Stable Sort Preservation" begin
        ...
        df = DataFrame(key = [1, 2, 1, 2, 1], order = [1, 2, 3, 4, 5])
...
@testset "Data Explorer - Histogram Computation" begin
    @testset "Histogram - Fixed Method" begin
        ...
        df = DataFrame(x = data)
        hist =
            Positron.compute_histogram(Positron.DataExplorerInstance(df, "test"), 1, params)
...
@testset "Data Explorer - DataFrame Column Types" begin
    ...
    @testset "Floating Point Types" begin
        df =
            DataFrame(float32 = Float32[1.1, 2.2, 3.3], float64 = Float64[1.11, 2.22, 3.33])
    ...
    @testset "String Columns" begin
        df = DataFrame(text = ["apple", "banana", "cherry", "date", "elderberry"])
    ...
    @testset "Date and Time Types" begin
        ...
        Positron.apply_sorting!(instance)
        @test df.date[instance.sorted_indices] ==
              [Date(2024, 1, 3), Date(2024, 1, 2), Date(2024, 1, 1)]
    end
    ...
    @testset "Empty Columns of Various Types" begin
        df = DataFrame(int = Int[], float = Float64[], string = String[], bool = Bool[])
...
@testset "Data Explorer - Frequency Tables" begin
    @testset "Frequency Table - Basic" begin
        ...
        freq = Positron.compute_frequency_table(
            Positron.DataExplorerInstance(df, "test"),
            1,
            params,
        )
    ...
    @testset "Frequency Table - Large Dataset" begin
        ...
        values = vcat(
            ...
            fill("B", 500),
            fill("C", 250),
            ["D$i" for i = 1:100],  # 100 unique rare values
        )
...
@testset "Data Explorer - Schema Operations Tests" begin
    ...
    @testset "julia_type_to_display_type - All Types" begin
        @test Positron.julia_type_to_display_type(Bool) ==
              Positron.ColumnDisplayType_Boolean
        @test Positron.julia_type_to_display_type(Int8) ==
              Positron.ColumnDisplayType_Integer
        @test Positron.julia_type_to_display_type(Int32) ==
              Positron.ColumnDisplayType_Integer
        @test Positron.julia_type_to_display_type(Int64) ==
              Positron.ColumnDisplayType_Integer
        @test Positron.julia_type_to_display_type(Float32) ==
              Positron.ColumnDisplayType_Floating
        @test Positron.julia_type_to_display_type(Float64) ==
              Positron.ColumnDisplayType_Floating
        @test Positron.julia_type_to_display_type(String) ==
              Positron.ColumnDisplayType_String
    end
...
@testset "Data Explorer - Profiles Respect Filters" begin
    ...
    @testset "Frequency Table on Filtered Data" begin
        df = DataFrame(category = vcat(fill("A", 50), fill("B", 30), fill("C", 20)))
...
@testset "Data Explorer - Filter and Sort Interactions" begin
    ...
    @testset "Filter Then Sort - Descending" begin
        ...
        instance.filtered_indices = [i for i = 1:20 if i % 2 == 0]
    ...
    @testset "Sort Then Filter" begin
        ...
        instance.filtered_indices = [i for i = 1:6 if df.value[i] > 3]
        Positron.update_view_indices!(instance)
    ...
    @testset "Multi-Column Sort with Filter" begin
        ...
        instance.filtered_indices = [i for i = 1:8 if df.category[i] in ["A", "B"]]
